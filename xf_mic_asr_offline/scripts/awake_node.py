...
# @Author: Aiden
# @Date: 2023/11/16
# 环形麦克风阵列Python SDK(circular Microphone Array Python SDK)
import re
import time
import json
import rclpy
...
class CircleMic:
    ...
    def switch_mic(self, mic="mic6_circle"):
        # mic：麦克风阵列类型，mic4：线性4麦，mic6：线性6麦， mic6_circle：环形6麦(mic: Microphone array type, mic4: Linear 4 mic, mic6: Linear 6 mic, mic6_circle: Circular 6 mic)
        param ={
            "type": "switch_mic",
            "content": {
                "mic": "mic6_circle"
    ...
    # 唤醒词更换（浅定制）(wake word replacement (shallow customization))
    def set_wakeup_word(self, str_pinyin="xiao3 huan4 xiao3 huan4"):
        # 参数为中文拼音(parameters are in Chinese Pinyin)
        # 更多参数请参考https://aiui.xfyun.cn/doc/aiui/3_access_service/access_hardware/r818/protocol.html(for more parameters, please refer to https://aiui.xfyun.cn/doc/aiui/3_access_service/access_hardware/r818/protocol.html)
        param = {
    ...
    # 数据串口发送(data serial transmission)
    def send_data(self, header, args):
        ...
        self.serialHandle.write(packet)  # 发送主控消息(send master control message)
        ...
        while True:
            ...
            if recv_data == header_[0]:
                ...
                if recv_data == header_[1]:
                    ...
                    else:  # 没有收到确认(no acknowledgment received)
                        recv_data = self.serialHandle.read(4)
                        self.serialHandle.read((recv_data[1] << 8 | recv_data[0]) + 1)

                        time.sleep(0.1)
                        self.serialHandle.write(
                            [0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00, 0xb0])  # 继续发送发送握手请求(continue to send handshake requests)
    ...
    # 检测是否唤醒以及唤醒对应角度(detect wake-up and corresponding angle)
                            ...
                            if m is not None:
                                ...
                                if m is not None:
                                    ...
                                    if self.flag_pub is not None and self.angle_pub is not None:
                                        ...
                                        angle = self.val_map(angle, 0, 360, 360, 0) + 240  # 和圆形兼容(compatible with circular)
                                        if angle >= 360:
                                            angle -= 360
                                        msg = Int32()
                                        msg.data = int(angle)

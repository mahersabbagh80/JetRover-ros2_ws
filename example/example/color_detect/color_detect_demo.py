...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓(iterate through all contours)
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 50:  # 只有在面积大于50时，最大面积的轮廓才是有效的，以过滤干扰(only contours with an area greater than 50 are considered valid, and among them, the contour with the largest area is selected to filter out interference)
                area_max_contour = c
...
def run(img):
    ...
    for i in ['red', 'green', 'blue']:
        frame_mask = cv2.inRange(frame_lab, tuple(lab_data['lab']['Stereo'][i]['min']), tuple(lab_data['lab']['Stereo'][i]['max']))  #对原图像和掩模进行位运算(Perform bitwise operations on the original image and mask)

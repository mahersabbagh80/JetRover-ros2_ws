...
class AutomaticPickNode(Node):
    ...
    def color_detect(self, img):
        ...
        if len(contours) != 0:
            areaMaxContour, area_max = common.get_area_max_contour(contours, 10)  # 找出最大轮廓(find out the contour with the maximum area)
            if areaMaxContour is not None:
                #print(111111111111, area_max)
                if 10 < area_max:  # 有找到最大面积(the maximum area is found)
                    ...
                    cv2.circle(img, (center_x, center_y), 5, (0, 255, 255), -1)  # 画出中心点(draw center point)
        ...
        if not self.pick or self.debug:
            ...
            if self.debug:
                ...
                if self.detect_count > 10:
                    ...
                    self.debug = False
                self.get_logger().info('x_y: ' + str([object_center_x, object_center_y]))  # 打印当前物体中心的像素(print the pixel coordinates of the current object's center)

...
class NavigationTransport(Node):
    ...
    def goal_callback(self, msg):
        ...
        if status == 'start' or status == 'place_finish':  # 处于可以pick的状态(it's in the state of picking)
            self.pick = True
            self.place = False
        ...
        elif status == 'pick_finish':  # 处于可以place的状态(it's in the state of placing)
            self.pick = False
            self.place = True

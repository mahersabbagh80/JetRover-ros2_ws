...
# @data:2022/11/07
# @author:aiden
# 肌体控制融合RGB(robot control fusion RGB)
import os
import cv2
import time
import queue
    ...
    def image_proc(self, image):
        ...
        if results is not None and results.pose_landmarks is not None:
            ...
            if self.count_akimbo > 5 and not self.calibrating:
                ...
                self.lock_color = None  # 标定颜色置空(calibrate robot to null)
            ...
            if self.count_no_akimbo > 5:
                ...
                self.count_akimbo = 0  # 检测归零(detect zeroing)
            ...
            if self.move_finish:
                ...
                if self.lock_color is None and self.current_color is not None and self.calibrating and not self.have_lock:
                    self.have_lock = True  # 标定好了(calibration is completed)
